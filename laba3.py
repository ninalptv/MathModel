...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.tree1 = ttk.Treeview(self, columns=("Интервал", "Теоретическая частота"), show="headings")
        ...
        for i in range(len(self.values1)):
            p = (scipy.stats.norm.cdf(app.xi[i + 1]) - 0.5) - (scipy.stats.norm.cdf(app.xi[i]) - 0.5) #теоритическая частота
            app.p.append(p)
            self.tree1.insert("", "end", values=(f"{app.xi[i]}-{app.xi[i + 1]}", p))
